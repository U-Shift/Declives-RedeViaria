#### Declives da rede viária, neste caso de Oeiras                   ####
##### Rosa Félix, 22 Outubro 2020                                    #####
...
ConcelhoLimite = Concelhos %>% filter(Concelho == "OEIRAS") #aqui podemos escolher outro qualquer
...
DEM = raster("raster/OeirasCOPERNICUS_clip.tif") #mudar o raster dependendo do município
crs(DEM) = CRS('+init=EPSG:3035') #3035 para ETRS89-LAEA
res(DEM)

raster::plot(DEM)
RedeViaria = st_transform(RedeViaria, 3035) #meter na projectao do DEM
...
#transformar novamente em 4326
RedeViaria = st_transform(RedeViaria, 4326) #meter na projectao do DEM

# exportar shapefile com os declives, em gpkg (QGIS) e kml (GoogleMaps)
st_write(RedeViaria, "shapefiles/RedeViariaOeiras_declivesEU.gpkg", append=F)
# st_write(RedeViaria, "shapefiles/RedeViariaBRaga_declives.kml", append=F)
...
tmap_save(mapadeclives, "html/DeclivesOeiras_EU.html") #exportar para html
