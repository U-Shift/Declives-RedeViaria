...
#### ATENÇÃO :coloquei tudo como eval=F porque é um processo ainda pesado para correr. basta remover isso e correr. ####
```
...
Para este passo é necessário usar o [package](https://ropensci.github.io/osmextract/articles/osmextract.html) `osmextract` que faz download do ficheiro mais actual do OSM que esteja disponível em https://download.geofabrik.de/index.html  
...
Se quiseremos uma rede mais _leve_, podemos escolher apenas as de categoria primary, secondary e tertiary (e respectivos links), que correspondem aos níveis mais elevados de uma rede viária.  
...
Todos estes processos fizeram com que o tipo de geometria da rede _limpa_ seja `GEOMETRY` ou `MULTILINESTRING`. Como o `slopes` só funciona com `LINESTRING`, vamos novamente pegar na rede OSM original e filtrar pelos `id` que ficaram na rede _limpa_.
...
![sem partir](figs/SplitLines_nodes.PNG?raw=true){ width=40% } ![depois de partir](figs/SplitLines_nodes_stplanr.PNG?raw=true){ width=40% }

Por outro lado, não queremos que sejam criados _nodes_ artificiais nos sítios onde duas linhas se cruzam, mesmo tendo elas diferentes níveis **z**, os tais _brunels_: _bridges and tunnels_.  O `v.clean` do GRASS (QGIS) acaba por parti-los :/ .O OSM representa as pontes e túneis como linhas que não se interssectam, e convém preservar esse formato.  
A função [`rnet_breakup_vertices`]() parte os segmentos nos seus vértices internos, preservando os _brunels_. Irá também aumentar o número de segmentos presentes na rede.  
```{r  message=FALSE, warning=FALSE}
nrow(RedeViaria)
...
![por baixo da ponte](figs/SplitLines_nodes_stplanrBridge.PNG?raw=true){ width=40% } ![nó do viaduto](figs/SplitLines_nodes_stplanrBridge2.PNG?raw=true){ width=40% }
